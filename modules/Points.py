...
##                self.pt[key]["y_geog"] += (iy - y_pix)  * pix
    ...
    """
    def network (self,targets, override_radius_pix=0):
         ...
         for pt1 in self.pt:
            ...
            if override_radius_pix: r = override_radius_pix
            else: r = self.pt[pt1]["radius"] #it's pixelised after take !!
...
##                   QgsFeatureRequest().setFilterFid(fid)).next()
        ...
        for feat in self.layer.getFeatures(QgsFeatureRequest( ids)):
            ...
            x_geog, y_geog = feat.geometry().asPoint()
